...
import { generateCauses, generateInterventions, validateDrivers } from './services/gemini';
...
const DefinitionView: React.FC<{
  ...
  addIntervention: (label: string, targets: string[], prerequisites: string[]) => void;
  driverValidation: DriverValidation | null;
  onValidateDrivers: () => void;
  isValidating: boolean;
}> = ({ state, setState, onNext, isGenerating, handleAiCauses, handleAiInterventions, addCause, addIntervention, driverValidation, onValidateDrivers, isValidating }) => {
  const [tempCause, setTempCause] = useState('');
  const [tempInt, setTempInt] = useState('');
  const [tempTargets, setTempTargets] = useState<string[]>([]);
  const [tempPrerequisites, setTempPrerequisites] = useState<string[]>([]);
    ...
    <div className="space-y-24 animate-in fade-in duration-500 pb-32">
      ...
      <section aria-labelledby="causes-title" className="grid grid-cols-1 lg:grid-cols-12 gap-16 border-t-4 border-swiss-black pt-12">
        ...
        <div className="lg:col-span-8 space-y-8">
          ...
          {/* OCI Assistant - Driver Validation */}
          {state.causes.length >= 2 && (
            <div className="space-y-4">
              <Button
                variant="secondary"
                onClick={onValidateDrivers}
                isLoading={isValidating}
                className="w-full justify-between group"
              >
                <span>OCI ASSISTANT // VALIDATE DRIVERS</span>
                <span className="bg-swiss-black text-white px-2 font-mono group-hover:bg-swiss-blue">✓</span>
              </Button>

              {driverValidation && (
                <div className="space-y-4 animate-in fade-in slide-in-from-top-4 duration-300">
                  {driverValidation.warnings.length === 0 && driverValidation.isValid ? (
                    <div className="bg-green-50 border-2 border-green-500 p-6">
                      <div className="flex items-start gap-4">
                        <div className="text-2xl">✓</div>
                        <div>
                          <div className="font-mono text-xs font-bold uppercase tracking-widest text-green-700 mb-2">Drivers Validated</div>
                          <p className="text-sm text-green-800 font-medium">All drivers appear independent and well-structured for OCI analysis.</p>
                        </div>
                      </div>
                    </div>
                  ) : (
                    <>
                      {driverValidation.warnings.map((warning: ValidationWarning, idx: number) => (
                        <div
                          key={idx}
                          className={`border-2 p-6 ${warning.severity === 'error'
                            ? 'bg-red-50 border-red-500'
                            : 'bg-yellow-50 border-yellow-500'
                            }`}
                        >
                          <div className="flex items-start gap-4">
                            <div className="text-2xl">{warning.severity === 'error' ? '⚠️' : '💡'}</div>
                            <div className="flex-1">
                              <div className={`font-mono text-xs font-bold uppercase tracking-widest mb-2 ${warning.severity === 'error' ? 'text-red-700' : 'text-yellow-700'
                                }`}>
                                {warning.type === 'hierarchical' && 'Hierarchical Relationship'}
                                {warning.type === 'causal' && 'Causal Dependency'}
                                {warning.type === 'redundant' && 'Redundancy Detected'}
                                {warning.type === 'overlap' && 'Overlap/Correlation'}
                              </div>
                              <p className={`text-sm mb-3 ${warning.severity === 'error' ? 'text-red-800' : 'text-yellow-800'
                                } font-medium`}>
                                {warning.message}
                              </p>
                              <div className="flex flex-wrap gap-2">
                                <span className="text-[10px] font-mono uppercase text-gray-500">Affected:</span>
                                {warning.driverIds.map((id: string) => {
                                  const driver = state.causes.find(c => c.id === id);
                                  return driver ? (
                                    <span key={id} className="text-[10px] font-mono font-bold uppercase px-2 py-1 bg-white border border-gray-300">
                                      {driver.label}
                                    </span>
                                  ) : null;
                                })}
                      ...
                      {driverValidation.suggestions.length > 0 && (
                        <div className="bg-swiss-blue/10 border-2 border-swiss-blue p-6">
                          <div className="font-mono text-xs font-bold uppercase tracking-widest text-swiss-blue mb-3">Suggestions for Improvement</div>
                          <ul className="space-y-2">
                            {driverValidation.suggestions.map((suggestion: string, idx: number) => (
                              <li key={idx} className="text-sm text-swiss-black flex items-start gap-2">
      ...
      <section aria-labelledby="int-title" className="grid grid-cols-1 lg:grid-cols-12 gap-16 border-t-4 border-swiss-black pt-12">
        ...
        <div className="lg:col-span-8 space-y-12">
          <div className="bg-swiss-gray/10 border-2 border-swiss-border p-8 space-y-8 relative">
            ...
            <div>
              <span className="block text-[10px] font-bold font-mono text-swiss-black mb-3 uppercase tracking-widest">Prerequisites (Optional)</span>
              <div className="flex flex-wrap gap-3" role="group" aria-label="Select prerequisites">
                {state.interventions.map(i => (
                  <button
                    key={i.id}
                    type="button"
                    onClick={() => {
                      setTempPrerequisites(prev => prev.includes(i.id) ? prev.filter(id => id !== i.id) : [...prev, i.id]);
                    }}
                    aria-pressed={tempPrerequisites.includes(i.id)}
                    className={`
                                px-4 py-3 text-[10px] font-mono uppercase tracking-widest border-2 transition-all duration-200
                                ${tempPrerequisites.includes(i.id)
                        ? 'bg-swiss-black text-swiss-white border-swiss-black font-bold shadow-sharp -translate-y-[2px]'
                        : 'bg-white border-swiss-border text-swiss-muted hover:border-swiss-black hover:text-swiss-black hover:shadow-hover focus-visible:border-swiss-black'
                      }
                              `}
                ...
                {state.interventions.length === 0 && <span className="text-swiss-muted font-mono text-xs italic px-2">No other interventions defined yet.</span>}
              </div>
            </div>
            <Button
              variant="primary"
              ...
              onClick={() => { addIntervention(tempInt, tempTargets, tempPrerequisites); setTempInt(''); setTempTargets([]); setTempPrerequisites([]); }}
                ...
                <div className="flex-1">
                  ...
                  {i.prerequisites && i.prerequisites.length > 0 && (
                    <div className="mt-2 flex flex-wrap gap-2">
                      <span className="text-[9px] font-mono font-bold uppercase text-swiss-muted mr-2">Requires:</span>
                      {i.prerequisites.map(pid => (
                        <span key={pid} className="text-[9px] font-mono font-bold uppercase px-2 py-1 border border-swiss-black bg-swiss-black text-white">
                          {state.interventions.find(prev => prev.id === pid)?.label}
                        </span>
                      ))}
                    </div>
                  )}
...
function App() {
  ...
  const [driverValidation, setDriverValidation] = useState<DriverValidation | null>(null);
  ...
  const addIntervention = (label: string, targetIds: string[], prerequisites: string[] = []) => {
    if (!label.trim()) return;
    setState(s => ({
      ...s,
      interventions: [...s.interventions, { id: uuidv4(), label, targetCauseIds: targetIds, prerequisites }]
    }));
  };
  ...
  // --- Driver Validation ---
  const handleValidateDrivers = async () => {
    if (state.causes.length < 2) return;
    setIsValidating(true);
    setDriverValidation(null);

    const result = await validateDrivers(state.outcome, state.causes);
    setDriverValidation(result as DriverValidation);
  ...
  const finalScores = useMemo(() => {
    // 1. Calculate Initial Base Scores
    let scores = state.interventions.map(intervention => {
    ...
    // 2. Propagate Scores based on Dependencies (Iterative approach to handle chains)
    // If B depends on A, then A's score should be at least B's score.
    // We iterate until stability or max iterations.
    let changed = true;
    let iterations = 0;
    while (changed && iterations < 10) {
      changed = false;
      // Create a map for quick lookup
      const scoreMap = new Map(scores.map(s => [s.id, s.baseScore]));

      scores = scores.map(item => {
        // Find all interventions that depend on this item
        const dependents = scores.filter(s => s.prerequisites?.includes(item.id));

        if (dependents.length > 0) {
          // Find the max score among dependents
          const maxDependentScore = Math.max(...dependents.map(d => scoreMap.get(d.id) || 0));

          if (maxDependentScore > item.baseScore) {
            changed = true;
            return { ...item, baseScore: maxDependentScore };
          }
        }
        return item;
      });
    ...
    return scores.sort((a, b) => b.baseScore - a.baseScore);
      ...
      <main className="flex-grow max-w-[1600px] mx-auto w-full px-6 md:px-12 py-16 print:p-0 print:max-w-none" role="main">
        ...
        {view === 'TOOL' && currentStep === Step.DEFINE && (
          <DefinitionView
            ...
            onValidateDrivers={handleValidateDrivers}
